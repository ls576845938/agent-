...
8. Rank candidates
9. Reject overfit
10. Promote to PAPER_ELIGIBLE (marks only, never auto-executes)
11. Generate dossier

NEVER auto-promotes to live. Max promotion: PAPER_ELIGIBLE marker.
"""
...
class ResearchAutomationPipeline:
    ...
    8. rank candidates
    9. reject overfit
    10. promote to PAPER_ELIGIBLE (marks, does not auto-execute)
    11. generate dossier
    ...
    def run(self, config: dict) -> dict:
        ...
        try:
            ...
            # Step 10: Promote clean candidates to PAPER_ELIGIBLE (mark only)
            promoted: list[str] = []
            for cid in clean_candidate_ids:
                self._mark_paper_eligible(cid)
                promoted.append(cid)
    ...
    def step_promote(self, candidate_id: str) -> StrategyCandidate:
        """Manually promote a candidate to PAPER_ELIGIBLE.

        This is a manual action -- requires explicit caller intent.
