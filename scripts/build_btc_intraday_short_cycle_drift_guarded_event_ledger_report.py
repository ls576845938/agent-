#!/usr/bin/env python3
"""Build the research-only BTC intraday drift-guarded event-ledger report."""
...
def main() -> None:
    ...
    parser.add_argument("--run-id", default=BTC_INTRADAY_DRIFT_GUARDED_EVENT_LEDGER_RUN_ID)
    ...
    run_dir = run_btc_intraday_short_cycle_drift_guarded_event_ledger(
        run_id=args.run_id,
        output_root=Path(args.output_root),
        latest_root=Path(args.latest_root),
        repo_root=Path(args.repo_root),
    )
    print(run_dir / "btc_intraday_short_cycle_drift_guarded_event_ledger_report.json")
