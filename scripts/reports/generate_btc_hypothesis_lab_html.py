#!/usr/bin/env python3
"""Generate a static BTC hypothesis lab HTML report."""
...
from quant_us.research.btc_hypothesis_lab import BTC_HYPOTHESIS_LAB_RUN_ID
...
def main() -> None:
    ...
    output = (
        Path(args.output)
        if args.output
        else Path("reports") / f"quantstation_vnext_btc_compression_expansion_hypothesis_{args.run_id}.html"
    )
...
def render_report(run_dir: Path) -> str:
    profile = _read(run_dir / "feature_profile.json")
    distribution = _read(run_dir / "distribution_report.json")
    decision = _read(run_dir / "hypothesis_decision.json")
...
<head>
  ...
  <title>QuantStation VNEXT - BTC Compression-to-Expansion Event Breakout Research Report</title>
  <style>
...
<body>
<header>
  <h1>QuantStation VNEXT - BTC Compression-to-Expansion Event Breakout Research Report</h1>
...
<main>
  ...
  <section>
    ...
    <ul>
      <li>This is research-only evidence generation; no paper candidate was created.</li>
      <li>perp_dual_trend remains archived and low-vol uptrend remains rejected.</li>
      <li>Order-flow is not used.</li>
    </ul>
  </section>
  ...
  <section>
    ...
    <ul>
      <li>perp_dual_trend archived after event_PF stayed near 1.01-1.02.</li>
      <li>low-vol uptrend was rejected with event_PF_proxy 0.979469 and fold pass rate 25%.</li>
      <li>compression-to-expansion was selected as the next hypothesis because it tests a different event-return state transition.</li>
    </ul>
  </section>

  <section>
    <h2>Hypothesis Lab Design</h2>
    <ul>
      <li>Config-driven hypothesis definition.</li>
      <li>Reusable feature profile, event table, distribution, gate, safety, and HTML contracts.</li>
      <li>Event-return objective with forward returns used only as labels.</li>
    </ul>
  </section>
  ...
  <section>
    ...
    <p>No-lookahead status: <code>{esc(profile.get("no_lookahead", {}).get("status", "unknown"))}</code></p>
  </section>

  <section>
    <h2>Event-Return Distribution</h2>
    <div class="grid">
      <div class="metric"><strong>Active events</strong><span>{esc(distribution.get("overall", {}).get("active_event_count"))}</span></div>
      <div class="metric"><strong>Selected direction</strong><span>{esc(distribution.get("selected_direction"))}</span></div>
      <div class="metric"><strong>event_PF_proxy</strong><span>{fmt(distribution.get("selected_direction_distribution", {}).get("event_PF_proxy"))}</span></div>
      <div class="metric"><strong>Fold pass rate</strong><span>{fmt(distribution.get("fold_stability", {}).get("pass_rate"))}</span></div>
    </div>
    {table([distribution.get("selected_direction_distribution", {})], ["active_event_count", "positive_event_rate", "mean_return", "median_return", "positive_sum", "negative_sum", "event_PF_proxy", "downside_tail_5pct"])}
  </section>

  <section>
    <h2>Direction Breakdown</h2>
    {table(_direction_rows(distribution.get("direction_breakdown", {})), ["direction", "active_event_count", "event_PF_proxy", "positive_event_rate", "mean_return", "median_return", "downside_tail_5pct"])}
  </section>

  <section>
    <h2>Fold Stability</h2>
    {table(distribution.get("fold_stability", {}).get("folds", []), ["fold_id", "active_event_count", "event_PF_proxy", "median_return", "positive_event_rate", "downside_tail_5pct", "passed"])}
  </section>

  <section>
    <h2>Horizon Analysis</h2>
    {table(_horizon_rows(distribution.get("horizon_analysis", {})), ["horizon", "active_event_count", "event_PF_proxy", "positive_event_rate", "mean_return", "median_return", "downside_tail_5pct"])}
  </section>

  <section>
    <h2>Tail Dependency</h2>
    {table([distribution.get("tail_dependency", {})], ["top5_positive_contribution", "top10_positive_contribution", "top5_negative_contribution", "edge_depends_on_extreme_events"])}
  </section>
...
def _direction_rows(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    rows = []
