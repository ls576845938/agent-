#!/usr/bin/env python3
"""Generate BTC liquidation-shock recovery hypothesis HTML report."""
...
from quant_us.research.btc_liquidation_shock_recovery import BTC_LIQUIDATION_SHOCK_RUN_ID
...
def main() -> None:
    ...
    output = (
        Path(args.output)
        if args.output
        else Path("reports") / f"quantstation_vnext_btc_liquidation_shock_recovery_{args.run_id}.html"
    )
...
def render_report(run_dir: Path) -> str:
    profile = _read(run_dir / "liquidation_shock_recovery_feature_profile.json")
    distribution = _read(run_dir / "liquidation_shock_recovery_distribution_report.json")
    decision = _read(run_dir / "liquidation_shock_recovery_hypothesis_decision.json")
...
<head>
  ...
  <title>QuantStation VNEXT - BTC Liquidation-Shock Recovery Research Report</title>
  <style>
...
<body>
<header>
  <h1>QuantStation VNEXT - BTC Liquidation-Shock Recovery Research Report</h1>
...
<main>
  ...
  <section>
    ...
    <ul>
      <li>This is research-only event-return evidence, not a paper candidate.</li>
      <li>perp_dual_trend stays archived; compression-expansion remains candidate_gate_failed.</li>
      <li>No broker API, no real orders, no paper auto-start, no live state change.</li>
    </ul>
  </section>
  ...
  <section>
    ...
    <div class="grid">
      <div class="metric"><strong>Active events</strong><span>{esc(overall.get("active_event_count"))}</span></div>
      <div class="metric"><strong>Primary horizon</strong><span>{esc(distribution.get("primary_horizon"))}</span></div>
      <div class="metric"><strong>event_PF_proxy</strong><span>{fmt(overall.get("event_PF_proxy"))}</span></div>
      <div class="metric"><strong>Median return</strong><span>{fmt(overall.get("median_return"))}</span></div>
    </div>
    {table([overall], ["active_event_count", "positive_event_rate", "mean_return", "median_return", "positive_sum", "negative_sum", "event_PF_proxy", "downside_tail_5pct"])}
  </section>

  <section>
    <h2>Fold Stability</h2>
    <p>Fold pass rate: <strong>{fmt(distribution.get("fold_stability", {}).get("pass_rate"))}</strong></p>
  ...
  <section>
    ...
    {table(distribution.get("regime_breakdown", []), ["regime", "active_event_count", "event_PF_proxy", "median_return", "positive_event_rate"])}
  </section>
